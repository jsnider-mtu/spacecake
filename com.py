        ...
        if len(args) != 0:
            if args[0] != "":
                ...
                with open("/root/spacecake/openai.json", "w") as f:
                    f.write(openaijson)
                conn.say(
                    "Your openai api key has been stored.\n"
...
def AddrFuncs(cmd, args, data, conn):
    ...
    if cmd.lower() == "whohere":
        ...
        msg = "Users in %s: %s %s" % (
            chan,
            " ".join(conn.nicks[chan]),
            str(len(conn.nicks[chan])),
        )
        print(msg)
    ...
    if re.match("s/.+/.*/", cmd):
            ...
            if re.fullmatch(r"\d*d\d+", die) != None:
                ...
                if n == "":
                    ...
                    for x in range(int(n)):
                        try:
                            results[die] += [
                                random.randint(1, int(die[die.index("d") + 1 :]))
                            ]
                        except KeyError:
                            results[die] = [
                                random.randint(1, int(die[die.index("d") + 1 :]))
        ...
        if data["sender"] == conn.trusted:
            ...
            try:
                ...
                conn.say(
                    "You need to register an api key to use this command\n"
                    "To register: /msg spacecake openai_register api_key",
                    chan,
                )
        if openai.api_key != "":
            if len(args) != 0:
                try:
                    ...
                    conn.say(
                        sendNick + ": Your api key is invalid\nTry to register again",
                        chan,
                    )
                except:
            ...
            conn.say(
                sendNick + ": Your api key is an empty string. Try to register again",
                chan,
            )
    elif cmd.lower() == ".texas" or cmd.lower() == "!texas":
        if len(args) == 0:
            conn.say(
                ".texas commands: newgame, list, join, start, bet, check, fold, hand, table, status, balance",
                chan,
            )
            return
        texasplayers = []
            ...
            if args[0].lower() == "newgame":
                if len(args) != 2:
                    ...
                    for z in texasgames:
                        if args[1] == z.name:
                            conn.say(
                                sendNick + f": The name {args[1]} is already taken",
                                chan,
                            )
                            return
                    ...
                    for z in texasgames:
                        if z.name == args[1]:
                            if z.player_join(poker.Player(sendNick)):
                                conn.say(
                                    f"Texas Hold 'Em game \"{z.name}\" has been created by {sendNick}. Use '.texas join {z.name}' to join",
                                    chan,
                                )
                            else:
                                conn.say(
                                    "Something went wrong starting a game (ノ°▽°)ノ︵┻━┻",
                                    chan,
                                )
                ...
                if len(args) != 2:
                    ...
                    for z in texasgames:
                        if args[1] == z.name:
                            if z.player_join(poker.Player(sendNick)):
                                conn.say(
                                    f'{sendNick} has joined the "{z.name}" Texas Hold \'Em game',
                                    chan,
                                )
                                return
                            else:
                                conn.say(
                                    f"The {z.name} Texas Hold 'Em game is full, sorry {sendNick}",
                                    chan,
                                )
                ...
                conn.say(
                    sendNick
                    + ": You aren't currently in a game. Use '.texas join <name>' to join a game",
                    chan,
                )
                ...
                for y in x.table.seats:
                    if y.is_filled():
                        if y.p.name == sendNick and y.p.turn != True:
                            if time.time() - x.last_turn > 900:
                                for z in x.table.seats:
                                    if z.is_filled():
                                        if z.p.turn == True:
                                            conn.say(
                                                f'{z.p.name} has been kicked from the "{x.name}" table for inactivity',
                                                chan,
                                            )
                                            x.player_leave(z.p)
                                            if (
                                                ...
                                                return
                                            if x.table.seats_taken() == 0:
                                                texasgames.remove(x)
                                                del x
                                                return
                                            x.last_turn = TexasNextTurn(x, conn, chan)
            if args[0].lower() == "newgame" or args[0].lower() == "join":
                conn.say(
                    sendNick
                    + ": Must leave your current game before creating or joining a new one",
                    chan,
                )
                    ...
                    for y in x.table.seats:
                        if y.is_filled():
                            if y.p.name == sendNick:
                                for z in x.table.seats:
                                    if z.is_filled():
                                        if z.p.purse < x.table.big_blind:
                                            if x.player_leave(z.p):
                                                conn.say(
                                                    f"{z.p.name} has insufficient funds and has left the {x.name} table",
                                                    chan,
                                                )
                                            else:
                                                conn.say(
                                                    "Something went wrong in playerleave() (ノ°▽°)ノ︵┻━┻",
                                                    chan,
                                                )
                                        else:
                                            z.just_sat = False
                                if x.running == True:
                                    conn.say(
                                        sendNick
                                        + f': Your game "{x.name}" is in the middle of a hand',
                                        chan,
                                    )
                                elif x.table.is_ready():
                                    x.running = True
                                    cardsneeded = (x.table.in_play() * 2) + 8
                                    ...
                                    for c in x.table.seats:
                                        if c.is_filled():
                                            if c.just_sat == False:
                                                conn.say(
                                                    f"Your hand: {c.p.hand}", c.p.name
                                                )
                                    x.last_turn = TexasNextTurn(x, conn, chan)
                                else:
                                    conn.say(
                                        f'Not enough players to start game "{x.name}"',
                                        chan,
                                    )
                        ...
                        if y.is_filled():
                            if y.p.name == sendNick:
                                if y.p.turn == True:
                                    ...
                                    x.last_turn = TexasNextTurn(x, conn, chan)
                                x.player_leave(y.p)
                                if x.table.in_play() == 1 and x.running == True:
                                    TexasWinCalc(x, conn, chan)
                                    return
                                if x.table.seats_taken() == 0:
                                    texasgames.remove(x)
                ...
                if len(args) != 2:
                    conn.say(
                        sendNick
                        + ": Usage: '.texas bet <integer>' (floating point numbers will be truncated)",
                        chan,
                    )
                    return
                for x in texasgames:
                    for y in x.table.seats:
                        if y.is_filled():
                            if (
                                y.p.name == sendNick
                                and y.just_sat == False
                                and y.p.folded == False
                                and y.p.turn == True
                            ):
                                try:
                                    diff = int(args[1]) - y.p.last_bet
                                    if int(args[1]) >= y.p.min_bet:
                                        if diff <= y.p.purse:
                                            if y.p.bet(int(args[1]), diff):
                                                x.table.pot.add(int(args[1]), diff)
                                                conn.say(
                                                    sendNick
                                                    + f" has just bet ${int(args[1])}",
                                                    chan,
                                                )
                                                x.player_turn += 1
                                                TexasBettingRound(x, conn, chan)
                                                if x.table.pot.last_bet == 0:
                                                    x.player_turn = 3
                                                    if x.running == True:
                                                        x.last_turn = TexasNextTurn(
                                                            x, conn, chan
                                                        )
                                                break
                                            else:
                                                conn.say(
                                                    "Something went wrong in bet() (ノ°▽°)ノ︵┻━┻",
                                                    chan,
                                                )
                                        else:
                                            conn.say(
                                                sendNick
                                                + f": ${int(args[1])} is more than you have to bet (you have ${y.p.purse})",
                                                chan,
                                            )
                                    else:
                                        conn.say(
                                            sendNick
                                            + f": ${int(args[1])} is less than the minimum bet of ${y.p.min_bet}",
                                            chan,
                                        )
                                except ValueError:
                                    conn.say(
                                        sendNick + f": {args[1]} is not an integer",
                                        chan,
                                    )
                        ...
                        if y.is_filled():
                            if (
                                ...
                                if y.p.check():
                                    ...
                                    conn.say(
                                        sendNick
                                        + f": you cannot check here, minimum bet is ${y.p.min_bet}",
                                        chan,
                                    )
                        ...
                        if y.is_filled():
                            if (
                                ...
                                if y.p.fold():
                                    ...
                                    conn.say(
                                        "Something went wrong in fold() (ノ°▽°)ノ︵┻━┻",
                                        chan,
                                    )
                    ...
                    for y in x.table.seats:
                        if y.is_filled():
                            if len(args) == 2:
                                if y.p.name == args[1]:
                                    conn.say(
                                        args[1] + f" has a balance of ${y.p.purse}",
                                        chan,
                                    )
                            else:
                                if y.p.name == sendNick:
                                    conn.say(
                                        sendNick + f" has a balance of ${y.p.purse}",
                                        chan,
                                    )
...
def TexasNextTurn(game, conn, chan):
    curturn = game.player_turn % game.table.in_play()
    c = -1
    for b in game.table.seats:
        if b.is_filled():
            if b.just_sat == False and b.p.folded == False:
                c += 1
                if c == curturn:
                    conn.say(
                        f"Player {b.p.name}'s turn. Current bet is ${game.table.pot.last_bet}",
                        chan,
                    )
                    b.p.turn = True
                    turn_time = time.time()
                    if b.p.has_bet == False:
                        if game.table.pot.last_bet == 0:
                            b.p.min_bet = 0
                        elif game.table.pot.last_bet >= b.p.purse:
                            b.p.min_bet = b.p.purse
                        else:
                            b.p.min_bet = game.table.pot.last_bet
                    elif b.p.last_bet < game.table.pot.last_bet:
                        b.p.min_bet = game.table.pot.last_bet
                    else:
    ...
    for b in game.table.seats:
        if b.is_filled():
            if b.just_sat == False and b.p.folded == False:
                c += 1
                if c == curturn:
                    if b.p.has_bet and b.p.last_bet == game.table.pot.last_bet:
                        conn.say(
                            f"Betting round over, current pot is ${game.table.pot.pot}",
                            chan,
                        )
                        game.table.clean()
                        if len(game.table.comm.flop_cards) == 0:
                            flopcardsmsg = "The Flop:"
                            game.table.comm.flop(game.d)
                            for c in game.table.comm.flop_cards:
                                flopcardsmsg += f" {c};"
                            conn.say(flopcardsmsg, chan)
                            conn.say(game.table.comm.cards(), chan)
                            break
                        elif game.table.comm.turn_card == None:
                            game.table.comm.turn(game.d)
                            conn.say(f"The Turn: {game.table.comm.turn_card}", chan)
                            conn.say(game.table.comm.cards(), chan)
                            break
                        elif game.table.comm.river_card == None:
                            game.table.comm.river(game.d)
                            conn.say(f"The River: {game.table.comm.river_card}", chan)
                            conn.say(game.table.comm.cards(), chan)
                            break
                        else:
...
def TexasWinCalc(x, conn, chan):
    ...
    x.table.deep_clean()
    x.dealer += 1
    if x.dealer == x.table.seats_taken():
        x.dealer = 0
    x.player_turn = 3 + x.dealer
    x.running = False
